export class ComponentManager {
  ...
  #allComponentsLoaded = () => {
    ...
    this.#components.forEach((c) => {
      if (c.hasOwnProperty("main")) ++markedAsLoaded;
    });
  ...
  load(component) {
    ...
    let componentIndex = self.#components.findIndex(
      (c) => c.src == component.src
    );
    if (componentIndex == -1) {
    ...
    return new Promise(function (doneLoading) {
      import(component.src).then(function (loadedComponent) {
        const componentIndex = self.#components.findIndex(
          (c) => c.src == component.src
        );
        ...
        if (newComponentInstance.name != component.name) {
          console.info(
            `componentManager: name of component differs from ${component.name} to ${newComponentInstance.name}! setting it to ${component.name}.`
          );
  ...
  run(componentName, args = {}) {
    ...
    if (!component.hasOwnProperty("app")) {
      this.load(component).then((newComp) => {
        newComp.main(args);
      });
      return true;
    }
